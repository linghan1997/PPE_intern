...
class CentroidTracker():
    def __init__(self, maxDisappeared=50):
        # initialize the next unique object ID to along with two ordered dictionaries used to keep track of
        # mapping a given object ID to its centroid and number of consecutive frames it has been marked
        # as "disappeared", respectively
        self.nextObjectID = 0
        # k: objectID v: centroid coordinate(x, y)
        self.objects = OrderedDict()
        # k: objectID v: number of consecutive frames a particular objectID has been marked as "lost"
    ...
    # adding new objects to the tracker
    def register(self, centroid):
        # when registering an object we use the next available object ID to store the centroid
        self.objects[self.nextObjectID] = centroid
        self.disappeared[self.nextObjectID] = 0
        self.nextObjectID += 1  # wait for another new object

    def deregister(self, objectID):
        # to deregister an objectID we delete the objectID from both of our respective dictionary
        del self.objects[objectID]
        del self.disappeared[objectID]

    # rects come from an object detector
    # the structure of rects may be a tuple as (startX, startY, endX, endY)
    def update(self, rects):
        ...
        # shape of inputCentroids = #bounding_boxes * 2 (coordinates)
        ...
        # otherwise, try to match the input centroids to existing objects centroids
        else:
            ...
            # 2. sort the row indexes based on their minimum values so that the row with the smallest value
